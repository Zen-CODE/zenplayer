...
class MusicLib(EventDispatcher):
    ...
    source = "/home/fruitbat/Music"  # PC. Linux
    ...
    def __init__(self, **kwargs):
        ...
        self.albums = MusicLib._get_albums(MusicLib.source, [], MusicLib.max_albums)
    ...
    def get_row_item(self, index, controller):
        ...
        add_label(
            Label(
                text="[b][color=#FFFF00]{0} : {1}[/color][/b]".format(
        ...
        if len(album["images"]) == 0:
            ...
            for source in album["images"]:
                ...
                image.bind(
                    on_touch_down=lambda w, t: w.collide_point(*t.pos)
                    and controller.stop()
                )
                di.ids.images.add_widget(image)

        for warn in album["warnings"]:
            add_label(Label(text="[color=#FF0000]{0}[/color]".format(warn)))

        # Add tracks
        for k, track in enumerate(album["tracks"]):
            playing = bool(
                controller.playing_album == controller.album_index
                and controller.playing_track == k
            )
            pl_label = PlaylistLabel(
                controller=controller, text=track, album_index=index, track_index=k
            )
            add_label(pl_label)
            if playing:
...
class Controller(EventDispatcher):
        ...
        if not self.manual_stop:
            if args[1] == "stop":
                if self._set_next_track():
                    self.play_track(None, self.playing_album, self.playing_track)
    ...
    def play_track(self, pl_label=None, album_index=0, track_index=0):
        ...
        full_path = path.join(album["folder"], album["tracks"][self.playing_track])
...
class AlbumScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        self.controller = Controller(music_lib=self.music_lib, album_screen=self)
