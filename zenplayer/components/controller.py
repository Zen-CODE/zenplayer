...
class Controller(EventDispatcher):
    ...
    def on_file_name(self, _widget, value):
        ...
        if len(parts) > 2:
            ...
            Logger.info(
                "Track change: %s: %s - %s", self.artist, self.album, self.track
            )
    ...
    def play_random_album(self):
        ...
        self.stop()
        artist, album = self.library.get_random_album()
        self.playlist.add_files(self.library.get_path(artist, album), "replace")
        self.play_index(0)
    ...
    def save(self):
        ...
        self.store.put(
            "state", volume=self.volume, position=self.position, state=self.state
        )
